...
class RtmEnv : 
    ...
    def __del__(self):
        for nn in self.name_space :
            for hh in self.name_space[nn].rtc_handles :
                self.name_space[nn].rtc_handles[hh].__del__()
...
#		       'dataport.buffer.type':'ringbuffer',
		       'dataport.buffer.type':'nullbuffer',
...
class RtcHandle :
  ...
  def __del__(self) :
    for pp in self.inports :
        self.inports[pp].con.disconnect()
        print 'release inport', self.name, pp
    for pp in self.outports :
        self.outports[pp].con.disconnect()
        print 'release outport', self.name, pp
    for pp in self.services :
        self.services[pp].con.disconnect()
        print 'release service', self.name, pp
